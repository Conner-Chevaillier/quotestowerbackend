...
app.get('/quotes/:email', function (req, res) {
    ...
    queries.joinGetQuotebyEmail(req.params.email).then(response => {
        // res.send(response[0])
        console.log('fave quote response', response)
        res.send({ quoteArray: response })
    })
})
...
//DELETE BY ID ROUTE
app.delete('/quotes/:email/:id', (req, res) => {
    console.log('params', req.params.id, req.params.email)
    queries.deleteQuote(req.params.email, req.params.id).then(response => res.send(response))
