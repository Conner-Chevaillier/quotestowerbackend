...
module.exports = {
   ...
   joinGetQuotebyEmail(email) {
      ...
      return db('users').where('email', email).select('faveIDs')
         .then(ids => {
            ids[0].faveIDs.map(id => {
               console.log('id', id)
               promiseArr.push(db('quotes').where('quoteId', id).select('*').then(quote => quote[0]))
            })
   ...
   deleteQuote(email, id) {
      console.log('id', id)
      let tempFaveArr = []

      return db('users').where('email', email).select('faveIDs')
         .then(ids => {
            console.log('ids', ids[0].faveIDs, id)
            tempFaveArr = ids[0].faveIDs.filter(faveID => faveID.toString() !== id)
            console.log('temp', tempFaveArr)
            return db('users').where('email', email).update('faveIDs', tempFaveArr).returning('*')
         })
   }
